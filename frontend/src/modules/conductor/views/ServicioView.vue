<template>
  <ConductorLayout titulo="Registrar Servicio">
    <div v-if="!servicioEnCurso">
      <form @submit.prevent="iniciarServicio" class="card">
        ...
        <div class="form-group">
          <label>Precio ($)</label>
          <input v-model.number="form.precio" type="number" min="0" required placeholder="0" />
        </div>
    ...
    <div v-else class="card servicio-activo">
      ...
      <div class="servicio-info">
        ...
        <div class="info-row"><span class="label">Precio</span><span class="value price">${{ servicioEnCurso.precio }}</span></div>
...
const form = ref({ tipo_servicio_id: null, destino_texto: '', precio: 0 });
...
const iniciarServicio = async () => {
  loading.value = true;
  try {
    servicioEnCurso.value = await conductorStore.crearServicio(form.value);
  } catch (e) {
    alert(e.response?.data?.message || 'Error al crear servicio');
  } finally {
    loading.value = false;
  }
};
...
onMounted(async () => { tiposServicio.value = await conductorStore.fetchTiposServicio(); });
</script>
